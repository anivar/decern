...
It runs build, test, the cvc5 proofs, clippy, fmt, cargo-deny, and the no-lineage guard — the
...
- The bar is identical to a human PR: [`./scripts/verify.sh`](scripts/verify.sh) green (build, test,
  cvc5 proofs, clippy, fmt, cargo-deny, standards guard) and honest, scoped commits.
